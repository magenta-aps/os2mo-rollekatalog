...
def is_org_unit_excluded(
    org_unit: GetOrgUnitOrgUnitsObjectsCurrent
    | GetPersonEmployeesObjectsCurrentItusersEngagementsValiditiesOrgUnit,
    exclude_org_unit_level: UUID | None,
    exclude_org_units: list[UUID],
) -> bool:
    """Whether an org unit is excluded from sync.

    An org unit is excluded if it (or any of its ancestors) is listed in
    ``exclude_org_units`` or has the excluded ``org_unit_level``. This is the
    single source of truth for the exclusion decision, used both when syncing
    org units and when deciding whether a person's engagement should produce a
    position (so we never reference an org unit we don't sync).
    """
    excluded = set(exclude_org_units)

    def is_excluded(unit) -> bool:
        return unit.uuid in excluded or (
            exclude_org_unit_level is not None
            and unit.org_unit_level is not None
            and unit.org_unit_level.uuid == exclude_org_unit_level
        )

    return any(is_excluded(unit) for unit in (org_unit, *(org_unit.ancestors or [])))
