...
a changed configuration), use the `/trigger/all` endpoint.
...
You can safely clear the database in production as long as you do a full
synchronization as described above. It is highly recommended to set
`SYNC_ENABLED: false` before, so partial data is not written to Rollekatalog
before the integration has caught up with all the events.

See `os2mo_rollekatalog/api.py` for other helpful endpoints for debugging.
